...
global
{
	file boundary <- file("../includes/boundary.shp");
	file road_shape <- file("../includes/road.shp");
	file line <- file("../includes/movingline.shp");
	file in_point <- file("../includes/in.shp");
	file out_point <- file("../includes/out.shp");
	file wait_point <- file("../includes/wait.shp");
	file light <- file("../includes/light.shp");
	graph road_network;
	// Hình ảnh
	...
	init
	{
		...
		create in_pt from: in_point with:
		[direction::int(read('id')), intersect::int(read('intersect')), flow::float(read('flow')), motor_ratio::float(read('mratio')), car_ratio::float(read('cratio')), bus_ratio::float(read('bratio'))];
	...
	{
		//save (total_waiting_time) to: '../data_timefix/total_waiting.txt' type: text rewrite: false;
		save (total_bus_in) to: '../data_timefix/bus.txt' type: text rewrite: false;
		save (total_car_in) to: '../data_timefix/car.txt' type: text rewrite: false;
		save (total_motor_in) to: '../data_timefix/motor.txt' type: text rewrite: false;
	}
	// sinh phương tiện
...
species vehicles skills: [driving]
{
	...
	reflex disappear when: location = point(target)
	{
		...
		save (riding_time) to: '../data_timefix/riding_time.txt' type: text rewrite: false;
		save (waiting_number) to: '../data_timefix/waiting_number.txt' type: text rewrite: false;
		do die;
	}
...
species intersection
{
	...
	{
		save (intersect_waiting_time) to: '../data_timefix/intersect_waiting_time' + self.id + '.txt' type: text rewrite: false;
		save (number_of_vehicles_pass) to: '../data_timefix/intersect_pass' + self.id + '.txt' type: text rewrite: false;
	}
...
//		save(queue_length) to: '../data_timefix/max_queue_length_' + inter.id + '_' + self.direction + '.txt' type: text rewrite: false;
//	}
	action compute_queue_length{
		queue_length <- inter.count_motor_stop(inter, direction)
						+ 4*inter.count_car_stop(inter, direction)
						+ 10*inter.count_bus_stop(inter, direction);
		save(queue_length) to: '../data_timefix/max_queue_length_' + inter.id + '_' + self.direction + '.txt' type: text rewrite: false;
		save(queue_length) to: '../data_timefix/max_queue_length' + div(cycle, 180000) + '.txt' type: text rewrite: false;
	}
...
experiment final1hreal type: gui
{
	output
	{
